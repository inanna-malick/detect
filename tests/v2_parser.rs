...
fn test_complex_expression() {
    let result = RawParser::parse_raw_expr(
        r#"(path.filename == "test.rs" OR ext in [js, ts]) AND NOT size > 1mb"#,
    )
    .unwrap();

    let expected = RawTestExpr::and(
        RawTestExpr::or(
            RawTestExpr::string_predicate("path.filename", "==", "test.rs"),
...
fn test_complex_selectors() {
    let result = RawParser::parse_raw_expr("path.filename == test.rs").unwrap();
    let expected = RawTestExpr::string_predicate("path.filename", "==", "test.rs");
    assert_eq!(result.to_test_expr(), expected);
