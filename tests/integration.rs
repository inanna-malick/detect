...
async fn test_name_only() {
    Case {
        expr: "*foo*",  // Use glob pattern for filename matching
...
async fn test_path_only() {
    Case {
        expr: "bar/*",  // Glob pattern for files in bar directory
        // This will match files with paths like bar/something
...
async fn test_not_name_only() {
    Case {
        expr: "!*foo*",  // Use glob pattern negation
...
async fn test_name_and_contents() {
    Case {
        expr: "*foo* && foo",  // Files with foo in name AND contents
        expected: &["foo"],  // Only the file "foo" contains "foo"
        files: vec![f("foo", "foo"), f("bar/foo", "baz"), f("bar/baz", "foo")],
    }
    .run()
    .await
