...
async fn test_temporal_equality_operator() {
    ...
    fs::File::open(&old_file).unwrap().set_modified(week_ago).unwrap();
...
async fn test_temporal_midnight_boundary() {
    ...
    let now = Local::now();
    let today_start = now.date_naive().and_time(NaiveTime::from_hms_opt(0, 0, 1).unwrap());
    let yesterday_end = now.date_naive().pred_opt().unwrap()
    ...
    fs::File::open(&after_midnight).unwrap().set_modified(today_start_systime).unwrap();
    fs::File::open(&before_midnight).unwrap().set_modified(yesterday_end_systime).unwrap();
...
async fn test_temporal_all_time_units() {
    ...
    for (expr, expected) in test_cases {
        ...
        for file in expected {
            assert!(found.contains(&file.to_string()), 
                    "Expression '{}' should find '{}', but found: {:?}", expr, file, found);
        }
    }
}
