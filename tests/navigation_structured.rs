...
fn yaml_sets_equal(a: &[&yaml_rust::Yaml], b: &[yaml_rust::Yaml]) -> bool {
    ...
    for item_b in b {
        if !a.iter().any(|item_a| *item_a == item_b) {
            return false;
        }
    }
...
fn run_yaml_tests(test_cases: &[YamlNavCase]) {
    for case in test_cases {
        let path = parse_path(case.path)
            .unwrap_or_else(|e| panic!("Test '{}': Failed to parse path '{}': {:?}", case.name, case.path, e));
...
/// Helper to construct JSON boolean
...
/// Helper to construct TOML boolean
fn toml_bool(b: bool) -> toml::Value {
...
fn toml_table(pairs: Vec<(&str, toml::Value)>) -> toml::Value {
    let map: toml::map::Map<String, toml::Value> = pairs
        .into_iter()
        .map(|(k, v)| (k.to_string(), v))
        .collect();
...
fn test_yaml_recursive_descent() {
    let test_cases = vec![
        YamlNavCase {
            ...
            document: yaml_hash(vec![
                ("config", yaml_hash(vec![
                    ("database", yaml_hash(vec![
                        ("host", yaml_str("localhost")),
                    ])),
                ])),
            ]),
            path: "..host",
        ...
        YamlNavCase {
            ...
            document: yaml_hash(vec![
                ("db1", yaml_hash(vec![
                    ("host", yaml_str("server1")),
                ])),
                ("db2", yaml_hash(vec![
                    ("host", yaml_str("server2")),
                ])),
            ]),
            path: "..host",
        ...
        YamlNavCase {
            ...
            document: yaml_hash(vec![
                ("services", yaml_array(vec![
                    yaml_hash(vec![
                        ("ports", yaml_array(vec![yaml_int(8080), yaml_int(8081)])),
                    ]),
                    yaml_hash(vec![
                        ("ports", yaml_array(vec![yaml_int(9090)])),
                    ]),
                ])),
            ]),
            path: "..ports[*]",
